...
def fetch_openweather_with_retry(api_key, regions=REGION_COORDS, max_retries=3, backoff=2.0):
    """Wrapper around fetch_openweather_all with retry logic."""
    import time
    last_exc = None
    for attempt in range(max_retries):
        try:
            return fetch_openweather_all(api_key, regions)
        except Exception as e:
            last_exc = e
            if attempt < max_retries - 1:
                time.sleep(backoff * (attempt + 1))
    raise RuntimeError(f"OpenWeather fetch failed after {max_retries} attempts: {last_exc}")
